...
C        A FEW ENGINES DETERIORATE ON THE GAS PATH FASTER THAN FLEET
C        EXPERIENCE ALLOWS.  THESE ARE THE TREND CASES THE MODEL IN
C        STEP 040 IS THERE TO CATCH.
         IF (RANF(JSEED) .LT. 0.06)
     +      DEGR = 0.0155 + RANF(JSEED) * 0.0075
C
         DO 400 K = 1, NFLT
            N = NFLT - K
            IDAY = -2 * N
