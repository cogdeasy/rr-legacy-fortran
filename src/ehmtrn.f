...
C
      INTEGER I, J, JCUR, N, IRC, JD, NOBS, NENGT, NSHRT, IPAGE
      INTEGER NLINE, IFAM, NSTEEP
      INTEGER ENGFND
      REAL V(NFVAL), SLOPE, YINT, RSQ, VBMX, SOILC, SDEBR
      REAL SFUEL, SHOUR, FFACT, SFCD
      CHARACTER*8 CE
      CHARACTER*6 CFLT
...
C     ---- CONTROL BREAK - REDUCE THE WINDOW JUST ACCUMULATED ---------
      CALL TRNRED (JCUR, X, Y, NOBS, VBMX, SOILC, SDEBR, SFUEL,
     +             SHOUR, CFTAB, FFDATM, NFAM)
      NENGT = NENGT + 1
      JCUR = J
      NOBS = 0
      VBMX = 0.0
...
C
  300 CONTINUE
      IF (NOBS .GE. MAXOBS) GO TO 350
      NOBS = NOBS + 1
      X(NOBS) = FLOAT(NOBS)
      Y(NOBS) = V(5)
  350 CONTINUE
      IF (V(9) .GT. VBMX) VBMX = V(9)
      SOILC = SOILC + V(14)
      SDEBR = SDEBR + V(16)
      SFUEL = SFUEL + V(15)
      SHOUR = SHOUR + V(1)
...
C
 ...
 9020 FORMAT (' 0', 100('-') /
     ...
     +        ' 0EHM2030I STEP 030 COMPLETE')
      END
C
      SUBROUTINE TRNRED (J, X, Y, NOBS, VBMX, SOILC, SDEBR, SFUEL,
     +                   SHOUR, CFTAB, FFDATM, NFAM)
C     ----------------------------------------------------------------
...
C     ----------------------------------------------------------------
      ...
      INCLUDE 'EHMCOM.INC'
      INTEGER J, NOBS, NFAM
      REAL X(*), Y(*), VBMX, SOILC, SDEBR, SFUEL, SHOUR
      REAL FFDATM(NFAM)
      CHARACTER*4 CFTAB(NFAM)
      INTEGER K, IFAM
