...
C     AND 'NEW' FOR AN OUTPUT FILE.  AN OPEN FAILURE IS FATAL - THE
C     JOB IS ABANDONED WITH A CONDITION CODE OF 16 SO THAT THE
C     FOLLOWING STEPS ARE FLUSHED.
C     ----------------------------------------------------------------
      INCLUDE 'EHMPRM.INC'
      INTEGER LU, IOS
...
C
     ...
     +      IOSTAT=IOS)
      GO TO 200
  100 CONTINUE
      OPEN (UNIT=LU, FILE=FNAME, STATUS='OLD', FORM='FORMATTED',
     +      IOSTAT=IOS)
  200 CONTINUE
